...
import { WeeklyScheduleRows } from "@/src/types/period";
...
export const getActiveSchedule =
  async (): Promise<WeeklyScheduleRows | null> => {
    const connection = await redisConnection;

    const schedule = await connection.hGet(
      APPLICATION_KEY,
      ACTIVE_SCHEDULE_KEY
    );
...
export const storeNewRecipe = async (recipe: Recipe) => {
  const connection = await redisConnection;
  return await connection.json.arrAppend(
    APPLICATION_KEY,
    RECIPES,
